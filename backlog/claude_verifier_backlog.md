...
This backlog turns the Claude hardening report into an executable enforcement pack against `~/.claude`.

> **STATUS AUDIT 2026-07-13 (final)**: FULLY IMPLEMENTED. All 14 backlog verifiers
> plus the scorecard-referenced `verify_claude_runtime_portability.py` (15 modules)
> now exist in `execution/`, each with a hermetic test suite (`.tmp`-sandboxed via
> `CLAUDE_RUNTIME_ROOT`) and each runs live against `~/.claude`.
> `score_claude_architecture.py` now measures **100 of 100 weight** (was 32) from
> live verifier evidence and earns **80** — the 20-point gap is the two categories
> zeroed by REAL `~/.claude` debris, each explained with a file-backed reason:
>   - `path_authority` (-10): `scripts/ps1_registration_nudge.py` pins `~/.claude`
>   - `runtime_coupling_boundaries` (-10): `~/.gemini/antigravity` refs in
>     `safety/hook_policies.json` + `scripts/retention_reaper.py`; `scripts/backup_scheduler.py` → `backups/`
> Those three findings need release-lane fixes (protected `~/.claude`); once fixed
> the score rises to 100 with no code change. Structural pair drift is caught by
> `tests/test_policy_pair_parity.py`. **NOT wired into `execution/doctor.py`'s hard
> gate** deliberately: the pack FAILs on the above real debris, and wiring it now
> would turn the repo dashboard red for issues Order Samurai cannot fix from here —
> wire it after the release-lane fixes land. Implementation notes retained for
> maintenance: treat `forbiddenPatterns` as plain substrings
> (`verify_no_stale_paths._literal_in`) or `re.escape` — raw `re.compile` crashes on
> the Windows entries; the shared `validate_root_hygiene_policy` rejects the claude
> classification vocabulary and resolves existence under REPO_ROOT.
...
- Runtime root: `~/.claude` (originally authored against the Windows host's `~/.claude`)
...
Acceptance criteria:
- one canonical target module for `~/.claude`
...
15. `verify_claude_runtime_portability.py` (scorecard-referenced; added 2026-07-13 to
    close the scorecard/backlog drift `verify_claude_pack_integrity.py` surfaced)

## Definition Of Done

The Claude enforcement pack is considered operational when:  **[MET 2026-07-13 — see STATUS AUDIT]**

- the Claude-specific policy JSONs are consumed by code rather than only read by humans ✓
- Order Samurai can score Claude from live verifier evidence ✓ (100/100 weight measured)
