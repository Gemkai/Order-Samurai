...
2. SPAWN ALL 4 RONIN AGENTS simultaneously via Task tool — one Task call per pillar,
   each with subagent_type="ronin" (the write-capable dojo worker; NOT the read-only
   ...
   d. Result output path — ABSOLUTE, in the MAIN tree (NOT inside the worktree):
      <main_repo_root>/state/ronin_results/<slug>.json
      Resolve <main_repo_root> to your own cwd (ORDER_SAMURAI_ROOT) and pass the full
      absolute path. The ronin cd's into its worktree, so a relative
      "state/ronin_results/<slug>.json" would write inside the worktree — where Step D.1
      did NOT seed it and Step D.3 does NOT poll. It must land in the main tree, exactly
      where Step D.1 seeded it and Step D.3 polls it.
