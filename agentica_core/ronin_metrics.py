...
Rules to prevent silent drift between the two:
  - New DASHBOARD reducers/metrics go in the Governance copy
    (Governance/agentica_core/aggregate.py).
...
# _skill_promotions / _loop_breaker_fires RETIRED 2026-07-19 with the live-kernel
# retirement of Skill_Promotions and Loop_Breaker_Fires (metric-surface review
# Part E item 3): their JSONL/state sources are never written on this host, so
# both metrics were permanently dark — removal, never faking. Their REGISTRY rows
# are gone too (the kernel-drift orphan guard forbids frozen rows absent from the
# live kernel). _read_jsonl/_count_jsonl_records stay — other reducers still use them.
...
def _resolve_history_path(repo_root: Path) -> Path:
    # repo_root is the Order Samurai dir; telemetry history lives at <Agentica-OS>/Data/telemetry.
    return repo_root.parent.parent / "Data" / "telemetry" / "metrics_history.jsonl"
