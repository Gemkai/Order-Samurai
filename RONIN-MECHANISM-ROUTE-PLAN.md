...
**Status: APPLIED + ENFORCED (2026-07-18).** Edits 1–3 shipped earlier (mechanism blocks in
METRIC_CONFIG, TS mechanism-first spawn path, emit_event --kind/--routing-efficient; efficacy
keys split by `::mechanism` per §7.3). `REFLEX_REQUIRE_GRANT=true` enabled 2026-07-18 via the
`com.agentica.order-samurai-api` LaunchAgent (rollback: delete the env var from the plist and
`launchctl kickstart -k`). Deprecated_Deps intentionally has NO mechanism (2026-07-08 audit).
This supersedes `RONIN-MECHANISM-ROUTE-DRAFT.md`, which
...
- **Executor**: `api/src/reflex-engine.ts`.
  ...
  - ⚠️ The old `runFallback` targeted `bin/execute_remediation_gemini.py`, which never existed — removed 2026-07-12; quota/limit exits are now classified as `'timeout'` (incomplete) via `QUOTA_LIMIT_RE`, same as the turn-cap case.
