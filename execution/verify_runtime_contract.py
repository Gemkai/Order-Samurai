...
def _withdrawn_from_this_export(repo_root: Path) -> set[str]:
    """Paths `.export-withdrawn` says this distribution deliberately does not ship.

    The public export withdrew PROJECT.md and RONIN_SPEC.md on 2026-08-16 while the
    runtime contract still required them, so the exported doctor FAILed on an absence
    the export itself had chosen — a gate reporting the product's own policy back as a
    defect (audit 2026-09-01, finding B3). The manifest is generated by
    bin/extract_public.py and ships with the tree, so the two statements cannot drift.

    Absent manifest (the normal in-repo case) means nothing is withdrawn.
    """
    manifest = repo_root / ".export-withdrawn"
    try:
...
def run_checks(repo_root: Path = REPO_ROOT) -> list[dict[str, str]]:
    ...
    withdrawn = _withdrawn_from_this_export(repo_root)
    excused = sorted(p for p in missing_paths if p in withdrawn)
    missing_paths = [p for p in missing_paths if p not in withdrawn]
    if excused:
        results.append(_make_result(
            "WARN", "runtime-contract.artifacts.withdrawn",
            f"not required in this distribution — .export-withdrawn declares "
            f"{', '.join(excused)} deliberately unshipped",
        ))
    if missing_paths:
    ...
    if anti_drift_error:
        results.append(_make_result("ERROR", "anti_drift_policy.json", anti_drift_error))
    else:
