...
def parse_ai_review_findings(review_text: str, filename: str, reviewer_name: str) -> list[dict]:
    parsed_findings = []
    pattern = re.compile(
        r'\[(CRITICAL|HIGH|MEDIUM|LOW)\]\s*(?:Line[s]?\s*(\d+)(?:-\d+)?:?\s*)?(.*)',
        re.IGNORECASE
    )
    ...
    for line in review_text.splitlines():
        ...
        if match:
            ...
            title = rest
            desc = rest
            remediation = "Review AI guidance and apply recommended fix."
            ...
            current_finding = {
                ...
                "consensus": "ai-reviewer",
                "source": "ai-model",
            }
        elif current_finding:
            clean_line = re.sub(r"^[\-\*\s]+", "", line_str)
            if clean_line.lower().startswith("remediation:"):
                rem_text = re.sub(r"^remediation:\s*", "", clean_line, flags=re.IGNORECASE)
                if rem_text.strip():
                    current_finding["remediation"] = rem_text.strip()[:1200]
            elif clean_line.lower().startswith("description:"):
                desc_text = re.sub(r"^description:\s*", "", clean_line, flags=re.IGNORECASE)
                if desc_text.strip():
                    current_finding["description"] = desc_text.strip()[:1200]
...
def run_multi_model_consensus(target_files: list[tuple[str, str]]) -> dict:
    ...
    if gemini_key:
        models_active.append("Gemini 3.6 Flash (High)")
        models_active.append("Gemini 3.1 Pro")
    if shutil.which("claude"):
        models_active.append("Claude Fable 5")
    elif openai_key:
    ...
    def _worker(filename: str, code: str):
        ...
        if gemini_key:
            try:
                res = _call_gemini(code[:15000], filename, gemini_key, model="gemini-3.6-flash")
                with review_lock:
                    llm_reviews.append(f"**[Gemini 3.6 Flash (High)]** {filename}:\n{res}")
            except Exception as e:
                print(f"[repo-audit] Gemini 3.6 Flash review failed for {filename}: {e}", file=sys.stderr)
        ...
        if shutil.which("claude"):
            try:
                prompt_text = REVIEW_PROMPT_TMPL.format(filename=filename, code=code[:15000])
                proc = subprocess.run(
                ...
                if proc.returncode == 0 and proc.stdout.strip():
                    ...
                    lines_clean = [l for l in out_text.splitlines() if not l.startswith("Permission allow rule") and not l.startswith("Warning:") and not l.startswith("SessionEnd hook")]
                    clean_res = "\n".join(lines_clean).strip()
                    with review_lock:
                        llm_reviews.append(f"**[Claude Fable 5]** {filename}:\n{clean_res}")
            except Exception as e:
                print(f"[repo-audit] Claude Fable 5 review failed for {filename}: {e}", file=sys.stderr)
        elif openai_key:
            try:
                ...
                print(f"[repo-audit] GPT-4o review failed for {filename}: {e}", file=sys.stderr)

        # 4. Ollama fallback if no cloud reviews succeeded
        if not gemini_key and not openai_key and not shutil.which("claude"):
            try:
    ...
    for rev in llm_reviews:
        ...
        if header_match:
            rev_name = header_match.group(1).strip()
            fname = header_match.group(2).strip()
            rev_text = header_match.group(3).strip()
            extracted = parse_ai_review_findings(rev_text, fname, rev_name)
            ai_findings.extend(extracted)
...
def audit_repository(target_dir: Path, repo_url: str = "", review_tier: str = "auto") -> dict:
    ...
    if review_tier == "premium":
        ...
        findings.extend(consensus_res.get("findings", []))
