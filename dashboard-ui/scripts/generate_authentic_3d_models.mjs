...
// Node.js polyfill for FileReader (required by GLTFExporter)
...
// Utility: Export a scene and write as binary GLB
      ...
      (result) => {
        try {
          ...
          if (result instanceof ArrayBuffer) {
            ...
            console.log(`✅  ${filename}  (binary GLB, ${result.byteLength} bytes)`)
          } else {
            const json = JSON.stringify(result, null, 2)
            fs.writeFileSync(outPath, json)
            console.log(`✅  ${filename}  (glTF JSON, ${json.length} bytes)`)
          }
          resolve()
        } catch (e) {
          reject(e)
